...
| Feed snap suspended while the pullback lives | `html.pb-live` turns off the phone feed's `scroll-snap-type: y mandatory`; the class arrives at swap and leaves on demote | owner device report: with the monuments hidden the footer was the only snap point left, so every touch scroll was yanked back — one scroll law at a time |
